...
X5 <- Xagg
X5$Genotype <- factor(X5$Genotype,levels = c("HET","GKO","TG5"), ordered = T)
a = "FA"
b = "value"
c = "Genotype"
...
p0 <- ggbarplot(X5, x = a, y = b,
   ...
   scale_y_cut(breaks=c(2500), which=c(1, 3), scales=c(3, 3)) +
scale_y_continuous( expand = c(0,0)     
                       ) +
  coord_cartesian(ylim=c(0,0.75*max(X5$value, na.rm = T)))


p1 <- ggpar(p0, legend = "right", title = "Diglyceride Acyl Chains",legend.title = c, xlab = F, ylab = "Acyl chain sum") +
...
stat.test <- X5 %>% 
  ...
  stat.test$p.adj.signif <- stat.test$p.signif 
   ...
   stat.test <- stat.test %>%  add_xy_position(fun="mean_se",x=a, step.increase = 0.07)
